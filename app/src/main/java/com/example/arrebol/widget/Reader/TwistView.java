...
public class TwistView extends View {
	...
	private static final float AUTO_AREA_BOTTOM_RIGHT_X = 17 / 22F, AUTO_AREA_BOTTOM_RIGHT_Y = 20 / 24F;// 右下角和左侧自滑区域占比
	...
	private Region mRegionCurrent;// 当前页区域
	private Region mRegionNext;// 下一页区域
	private Region mRegionFold;// 折叠区域
	...
	private float mAutoAreaBottom, mAutoAreaTop, mAutoAreaRight, mAutoAreaLeft;// 右下角和左侧自滑区域
	private float mStart_X, mStart_Y;// ֱ直线起点坐标
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		 ...
		 * 计算自滑位置
		 */
		mAutoAreaBottom = mViewHeight * AUTO_AREA_BOTTOM_RIGHT_Y;
		mAutoAreaTop = mViewHeight - mAutoAreaBottom;
		mAutoAreaRight = mViewWidth * AUTO_AREA_BOTTOM_RIGHT_X;
	...
	private void slide() {
		...
		/*
		 * 如果当前页不是最后一页，如果是需要翻下一页，并且上一页已被做掉
		 */
		...
		/*
		 * 如果当前滑动标识位向左上滑动x坐标恒小于控件宽度
		 */
		else if(mSlide == Slide.RIGHT_TOP && mPointX < mViewWidth){
			// 则让x坐标自加
			mPointX += mAutoSlideV_BR;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 手指抬起时候
			...
			if (isNextPage) {
				...
				}else if(x > mAutoAreaRight && y < mAutoAreaTop){
					Log.d("zcc", "onTouchEvent: 右上滑");
					mSlide = Slide.RIGHT_TOP;
					justSlide(x, y);
				}else{
					if(oldX - x > 50F || x > (mViewWidth / 2.0)){//手势为从右往左滑动/点击的区域在屏幕右侧
						//向左翻页
